...
public class RankedChoiceElectionTest
{
    ...
    [Fact]
    public void ElectionWithAbsoluteMajorityInFirstPreference()
    {
        var voterOne = new SimpleVoter(1, "Voter one");
        var voterTwo = new SimpleVoter(2, "Voter two");
        ...
        var election = new RankedChoiceElection(ballots,  new List<ICandidate>() { candidateOne, candidateTwo });
        election.CountVotes();
        Assert.Equal(candidateOne, election.Winner);
    }

    [Fact]
    public void ElectionWithoutAbsoluteMajorityInFirstPreference()
    {
        ...
        Assert.DoesNotContain(candidateThree, election.Candidates);
        Assert.DoesNotContain(candidateTwo, election.Candidates);
        Assert.Equal(candidateOne, election.Winner);
        Assert.Equal(4, election.GetFirstPreferenceVotes(candidateOne));
    }
}